...
###   bastojicore.org and bastoji.org and compares them.
...
WORKINGDIR="/tmp/bastoji_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bastojicore.org"
HOST2="https://bastoji.org"
BASEDIR="/bin/"
VERSIONPREFIX="bastoji-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bastoji-',
...
if [ $? -ne 0 ]; then
   echo "bastoji.org failed to provide signature file, but bastojicore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bastoji.org and bastojicore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bastoji Core binary release signing key installed?"
   fi
