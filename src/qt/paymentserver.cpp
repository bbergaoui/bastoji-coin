...
const int BASTOJI_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BASTOJI_IPC_PREFIX("bastoji:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BASTOJI_IPC_PREFIX, Qt::CaseInsensitive)) // bastoji: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseBastojiURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BASTOJI_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BASTOJI_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bastoji://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bastoji://' is not a valid URI. Use 'bastoji:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BASTOJI_IPC_PREFIX, Qt::CaseInsensitive)) // bastoji: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBastojiURI(s, &recipient))
            {
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(BastojiUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
